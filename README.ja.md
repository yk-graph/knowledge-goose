...
- Node.js 24 LTS ── [Volta](https://volta.sh/) により `package.json` の `volta` フィールドで固定
- pnpm 10 以上 ── `package.json` の `packageManager` フィールドで固定、Corepack が管理
- 初回のみ `corepack enable` を実行してください(Node 標準同梱)
